...
async def update_primary_details(request:Request):
    ...
    update_details=data_table.query(applicant).filter(applicant.basic_details_id==id)
...
async def get_allprimary(request):
    details=data_table.query(applicant).all()
    content=[{k:v for k, v in vars(detail).items() if k!='_sa_instance_state'} for detail in details]
...
async def get_alladdress(request):
    ...
    content=[{k:v for k, v in vars(address).items() if k!='_sa_instance_state'} for address in details]
    print(content)
    return JSONResponse({'data':content})
...
async def create_education_details(request:Request):
    if request.method == "POST":
        ...
        education_data["passing_year"] = datetime.strptime(education_data["passing_year"], '%Y-%m-%d').date()
...
async def get_alleducation(request):
    ...
    content=[{k:v for k, v in vars(education).items() if k!='_sa_instance_state'} for education in details]
    print(content)
    return JSONResponse({'data':content})
...
async def get_allexperience(request):
    ...
    content=[{k:v for k, v in vars(experience).items() if k!='_sa_instance_state'} for experience in details]
    print(content)
    return JSONResponse({'data':content})
...
async def get_allskill(request):
    ...
    content=[{k:v for k, v in vars(skill).items() if k!='_sa_instance_state'} for skill in details]
    print(content)
    return JSONResponse({'data':content})
...
async def get_allproject(request):
    ...
    content=[{k:v for k, v in vars(project).items() if k!='_sa_instance_state'} for project in details]
    print(content)
    return JSONResponse({'data':content})
...
async def get_allsocial(request):
    ...
    content=[{k:v for k, v in vars(social).items() if k!='_sa_instance_state'} for social in details]
    print(content)
    return JSONResponse({'data':content})
...
def prim(request,id):
    content=data_table.query(applicant).filter(applicant.basic_details_id==id).first()
    print(content)
...
def skl(request,id):
    ...
    content=[{k:v for k, v in vars(skill).items() if k!='_sa_instance_state'} for skill in content]
    return content

def pro(request,id):
    content=data_table.query(projects).filter(projects.basic_details_id==id).all()
    content=[{k:v for k, v in vars(project).items() if k!='_sa_instance_state'} for project in content]
    return content
...
async def get_individual_resume(request):
    ...
    if request.method == "PUT":
        ...
        try:
            for pr in primary:
                primary_details_db = data_table.query(applicant).filter(applicant.basic_details_id == id)
...
async def new_resume(request):
    if request.method == "POST":
        ...
        for proj in project_data:
            ...
            print("New project :",project_data)
            try:
...
async def search(request:Request):
    ...
    search_name=data_table.query(applicant).filter((applicant.name == search_key) | (applicant.email == search_key)).all()
    content=[{k:v for k, v in vars(search_nam).items() if k!='_sa_instance_state'} for search_nam in search_name]
    content=[{k:str(v) if k == 'date_applied' else v for k,v in cont.items() } for cont in content]
...
routes=[
    ...
    Route('/delete_primary/{person_id:int}/',endpoint=delete_primary,methods=["DELETE"]),
    Route('/update_primary_details/{person_id:int}/',endpoint=update_primary_details,methods=["GET","PUT"]),
    Route('/get_allprimary',endpoint=get_allprimary),

    Route('/create_address_details/{person_id:int}',endpoint=create_address_details,methods=["GET","POST"]),
    Route('/update_address_details/{person_id:int}',endpoint=update_address_details,methods=["GET","PUT"]),
    Route('/delete_address/{person_id:int}/',endpoint=delete_address,methods=["DELETE"]),
    Route('/get_alladdress',endpoint=get_alladdress),

    Route('/create_education_details/{person_id:int}/',endpoint=create_education_details,methods=["GET","POST"]),
    Route('/update_educational_details/{person_id:int}/',endpoint=update_education_details,methods=["GET","PUT"]),
    Route('/delete_education/{person_id:int}/',endpoint=delete_education,methods=["DELETE"]),
    Route('/get_alleducation',endpoint=get_alleducation),

    Route('/create_experience_details/{person_id:int}/',endpoint=create_experience_details,methods=["GET","POST"]),
    Route('/update_experience_details/{person_id:int}',endpoint=update_experience_details,methods=["GET","PUT"]),
    Route('/delete_experience/{person_id:int}/',endpoint=delete_experience,methods=["DELETE"]),
    Route('/get_allexpereince',endpoint=get_allexperience),

    Route('/create_skill_details/{person_id:int}/',endpoint=create_skill_details,methods=["GET","POST"]),
    Route('/update_skill_details/{person_id:int}/',endpoint=update_skill_details,methods=["GET","PUT"]),
    Route('/delete_skill/{person_id:int}/',endpoint=delete_skill,methods=["DELETE"]),
    Route('/get_allskill',endpoint=get_allskill),

    Route('/create_project_details/{person_id}/',endpoint=create_project_details,methods=["GET","POST"]),
    Route('/update_project_details/{person_id:int}/',endpoint=update_project_details,methods=["GET","PUT"]),
    Route('/delete_project/{person_id:int}/',endpoint=delete_project,methods=["DELETE"]),
    Route('/get_allproject',endpoint=get_allproject),

    Route('/create_social_details/{person_id}/',endpoint=create_social_details,methods=["GET","POST"]),
    Route('/update_social_details/{person_id:int}/',endpoint=update_social_details,methods=["GET","PUT"]),
    Route('/delete_social/{person_id:int}/',endpoint=delete_social,methods=["DELETE"]),
    Route('/get_allsocial',endpoint=get_allsocial),


    Route('/get_individual_resume/{person_id:int}',endpoint=get_individual_resume,methods=["GET","PUT"]),
